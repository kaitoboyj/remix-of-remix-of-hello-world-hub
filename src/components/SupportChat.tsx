...
/**
 * Floating "Contact support" bubble, top-left on every page. Visible once the
 * wallet has moved more than $200 in total, or whenever an admin forces it on.
 */
export function SupportChat() {
  ...
  return (
    <div className="fixed left-4 top-4 z-[60] flex flex-col items-start gap-2">
      {!open && (
        <button
          type="button"
          onClick={openChat}
          className="relative inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-2 text-sm font-medium text-foreground shadow-lg transition hover:border-primary"
        >
      ...
      {open && (
        <div className="flex h-[26rem] w-[20rem] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-xl border border-border bg-card shadow-2xl">
